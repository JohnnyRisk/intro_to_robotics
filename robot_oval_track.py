...
def dist_to_circle(x,y,h,k,r):
    return sqrt((x-h)**2 + (y-k)**2) - r
# ------------------------------------------------
#
...
class robot:
    ...
    def cte(self, radius):
        if self.x <= radius:
            cte = dist_to_circle(self.x, self.y, radius, radius, radius)
        elif self.x > radius and self.x <= 3*radius:
            if self.orientation > 3*pi/2 or self.orientation <=pi/2:
                cte = self.y - 2*radius
            else:
                cte = -self.y
        else:
            cte = dist_to_circle(self.x, self.y, 3*radius, radius, radius)
        return cte
