...
class plan:
    ...
    # One improvement to make is to find fixed points and update this smooth function
    # need a way to algorithmically create fixed points
    # we can also change this such that it pushes away from objects
    def smooth(self, weight_data=0.1, weight_smooth=0.1,
        ...
        # This can be replaced with a deep copy
                ...
                for j in range(len(self.path[0])):
                    ...
                    self.spath[i][j] += weight_smooth * \
                                        (self.spath[i - 1][j] + self.spath[i + 1][j]
                                         - (2.0 * self.spath[i][j]))
                    # This is a minimization of a snap path and this is neccessary because it is noncyclical
                    if i >= 2:
...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # check that we have gone the whole segment
        if u > 1:
            index += 1
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
         weight_data, weight_smooth, p_gain, d_gain, printflag=False):
    path = plan(grid, init, goal)
    path.astar()
    path.smooth(weight_data, weight_smooth)
    return run(grid, goal, path.spath, [p_gain, d_gain],printflag)
...
def average_cost(params):
    weight_data, weight_smooth, p_gain, d_gain = params[0],params[1],params[2],params[3]
    cost=0
    for i in range(100):
        finished, collisions, steps = main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
            weight_data, weight_smooth, p_gain, d_gain)
        cost += (not finished)*10000 + collisions *100 + steps
    print('hi')
    print('{0:.0f} ,{1:.1f}, {2:.1f}, {3:.1f}, {4:.1f}'.format(cost,weight_data,weight_smooth,p_gain,d_gain))
    return cost/100
...
import numpy as np
x0 = np.array([0, 0, 1, 4])
bounds = np.array([(0,1),(0,1),(0,10),(0,10)])

res = minimize(average_cost, x0, method='L-BFGS-B',
               bounds=bounds,options={'maxiter':1e1,'disp': True})
'''
