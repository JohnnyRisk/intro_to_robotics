...
###################################
#### THIS IS INCORRECT BUT I THINK IT IS ACTUALLY A BETTER SOLUTION
#### The one below is the "correct" solution
##################################
    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            ...
            for j in range(len(path[0])):
                newpath[i%n][j] += weight_data * (path[i%n][j] - newpath[i%n][j]) +\
                                 weight_smooth * (newpath[(i+1)%n][j] + newpath[(i-1)%n][j] - 2*newpath[i%n][j])
