...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i, obs in enumerate(data):
        ...
        # we create indicies for x and y which because they alternate in the matrix
        j = 2 * i
        j_plus = j + 1

        #create temp variables of zeros and ifll them in
        omega_tmp = np.zeros_like(omega)
        mu_tmp = np.zeros_like(mu)

        # for the x-movement
        omega_tmp[j, j] = 1.0 / motion_noise ** 2
        omega_tmp[j, j + 2] = -1.0 / motion_noise ** 2
        omega_tmp[j + 2, j + 2] = 1.0 / motion_noise ** 2
        omega_tmp[j + 2, j] = -1.0 / motion_noise ** 2

        # for the y-movement
        omega_tmp[j_plus, j_plus] = 1.0 / motion_noise ** 2
        omega_tmp[j_plus, j_plus + 2] = -1.0 / motion_noise ** 2
        omega_tmp[j_plus + 2, j_plus + 2] = 1.0 / motion_noise ** 2
        omega_tmp[j_plus + 2, j_plus] = -1.0 / motion_noise ** 2

        #update our omega we can clean it up by just dividing the whole matrix by the movement noise
        omega += omega_tmp

        #we do the same thing here with the mu
        mu_tmp[j, 0] = -x_move / motion_noise ** 2
        mu_tmp[j + 2, 0] = x_move / motion_noise ** 2
        mu_tmp[j_plus, 0] = -y_move / motion_noise ** 2
        mu_tmp[j_plus + 2, 0] = y_move / motion_noise ** 2
        mu += mu_tmp

        # go through the landmarks and take in the observations
        for landmark in landmarks:
            ...
            # the last return is structured this way to get out of numpy and into
            # the matrix class that has the printing function
            # i WAS JUST lazy and did not want to make a new printing function
